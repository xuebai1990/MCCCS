...
\noindent{\bf ltraj:} Yields the trajectory file (file\_traj,
or fort.12 by default) if TRUE.
...
\noindent{\bf lrecip:} if TRUE an efficient way to calculate
cosine and sine for reciprocal space energy is enabled. (Warining:
does not work for non-rectangular box. Use with caution when 
running with more than one processor, since it may be even slower
...
\noindent{\bf lkdtree:} Kd-tree data structure is used to
speed up the simulation. It supports bead based kd-tree (lcutcm = F)
and center-of-mass based kd-tree (lcutcm = T).
It provides speed up for large system size
(e.g. interaction sites greater than 6,000).
If the molecule is small and rigid (e.g. water),
center-of-mass kd-tree performs better than bead kd-tree.
However, if the molecule is large, bead-based kd-tree is a better choice.
False by default.
...
\noindent Note that the unit for equilibrium dihedral angle ($vtt1$) is radian!
...
conditions). This should be set to -1 for production
(the default).

\noindent{\bf l\_bilayer:} Allow special volume moves in box 1.
requires lsolid and lrect be set to true and false, respectively,
for box 1.

\noindent{\bf pm\_consv:} Used when l\_bilayer is true.
...
\noindent{\bf pmvol\_xy:}  Used when l\_bilayer is true.
...
\subsection{Section \textbf{KDTREE}}
Parameters for setting up Kd-tree data structure. Only read
if lkdtree in topmon.inp is TRUE.

The section has four lines, with the first "KDTREE" and the
last "END KDTREE". The second line indicates whether
the simulation box uses kdtree, while the third line
indicates the length of the buffer region used in the data
structure.

The buffer region length should be
greater than the largest distance between center-of-mass
and any bead in this molecule. Otherwise, there can be
errors related to Kd-tree, or energy errors.
The efficiency of the simulation decreases as the buffer
region size increases, as more beads in the periodic
boundary conditions need to be stored.

\noindent {\bf Example}\hfill\break
Kdtree section setup for two-box $NVT$-Gibbs simulations,
with both boxes use Kd-tree and the buffer region sizes
are 5 \AA.
