...
move types. You want to chose values covering a range that 
will not arbitrarily cut off the tails when looking at a bond 
length distribution, but narrow enough to not significantly 
impact the efficiency of a simulation. The easiest way to 
determine a good range on these values is to run a short and 
simple simulation of of just the molecule of interest with a 
very wide range of allowed values. View the resulting bond 
length distribution and determine reasonable ranges from this. 
Caution is still necessary as in a full simulation the average 
bond length is not necessarily equal to the equilibrium bond 
length. In some simulations the center of your distribution will 
be shifted away from the equilibrum value. So again even after 
you have selected values, you should check to ensure that you 
are not arbitrarily cutting off your bond length distribtuon. 
Common values in the past have been 0.9-1.1. 
