...
cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
      ...
      parameter (bmax=20000,nmax=3000,numax=20,ntmax=12,nbxmax=3)
      parameter (nntype=600,nvmax=30,fmax=100)
...
C     Read bead coordinates
            ...
            do i=1,nbox
               ...
               write(6,*)'box ',i,' x=',boxlx(i),' y=',boxly(i),
     &              ' z=',boxlz(i)
               print*,'no of molecules: ',(ncmt(i,j),j=1,nmolty)
...
cccc  set to zero each time
     ...
     &              nunit(imolty(i)),nboxi(i),xcm(i),ycm(i),zcm(i)
               print*,'molecule ',i,':',idum,imolty(i),
     &              nunit(imolty(i)),nboxi(i),xcm(i),ycm(i),zcm(i)
               if (idum.ne.i) stop 'inconsistent moview file' 
               do ii=1,nunit(imolty(i))
                  read(10,*) xcoord(i,ii),ycoord(i,ii),
     &                 zcoord(i,ii),ddum,beadty(imolty(i),ii)
                  print*,'unit ',ii,':', xcoord(i,ii),ycoord(i,ii),
     &                 zcoord(i,ii),ddum,beadty(imolty(i),ii)

c                  if (imolty(i).eq.2) then
c                     beadty(imolty(i),ii) = 45
c                  endif
...
c        switch beadtype for C6 (moltyp 2)      
c            do i=1,nchain
c               do ii=1,nunit(imolty(i))
c                  if (imolty(i).eq.2) then
c                     beadty(imolty(i),ii) = 40
c                  endif      
c               enddo
c            enddo
