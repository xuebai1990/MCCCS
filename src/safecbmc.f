...
!$$$      include 'cbmc.inc'
      ...
      real(KIND=double_precision)::vdha ,phicrank,bf_tor,vtorsion,vbend,rbf,ran_tor ,bs,ang_bend,bfactor,bsum_bend,wei_bv,bsum_try,third,vibtr

!     *** to conserve memory, max is the maximum number of endpoints
!     *** possible in place of numax
      parameter(max=10)

      real(KIND=double_precision)::flength,x,y,z,equil,kforce,length ,vvib,equilb,kforceb,ux,uy,uz,hdist,lengtha,lengthb ,vtor,vphi ,thetac,angle,equila,kforcea,ovphi ,alpha,phidisp,dum,random,rxt ,ryt,rzt ,phiacc,rxa,rya,rza,angles,bangles,vctor ,r,mincb,delcb ,vkforce,vequil,vvibration,ovvib
