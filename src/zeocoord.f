...
!$$$      include 'mpi.inc'
      ...
      if (myid.eq.0) write(16,"(/ ,' READING ZEOLITE LATTICE FROM FILE zeolite.cssr:',/ ,' --------------------------------------------------')")
      ...
      if (myid.eq.0) write(16,"(' box angles                        = ',3f10.3,' degrees')") alpha,betaang,gamma
      ...
      if (myid.eq.0) then
         write(16,"(  ' box dimensions                    = ',3f10.3, ' Angstrom',/, ' simulation volume                 = ', f10.1, 20x,' Angst**3')") zeorx,zeory,zeorz,cell_vol(ibox)
      end if

      read(47,*)   nzeo,frac,nx,ny,nz

      if (myid.eq.0) write(16,"(' number of zeolite atoms           = ',i10)") nzeo
...
!     === Converting to absolute coordinates within [0,ri>
      ...
      znum=0
      do izeo = 1,nzeo
         read(47,'(i5,1x,a4,2x,3(f9.5,1x))') count,atom,zeox(izeo) ,zeoy(izeo),zeoz(izeo)
         sx = zeox(izeo)*hmati(ibox,1)+zeoy(izeo)*hmati(ibox,4) +zeoz(izeo)*hmati(ibox,7)
         sy = zeox(izeo)*hmati(ibox,2)+zeoy(izeo)*hmati(ibox,5) +zeoz(izeo)*hmati(ibox,8)
         sz = zeox(izeo)*hmati(ibox,3)+zeoy(izeo)*hmati(ibox,6) +zeoz(izeo)*hmati(ibox,9)
         sx = sx - floor(sx)
         sy = sy - floor(sy)
         sz = sz - floor(sz)
...
!     === Calculate zeolite density from assumption no of Si = 0.5* no O
      ...
      if (myid.eq.0) write(16,"(' mass zeolite                      = ',e10.3,' grams',/ ,' one adsorbed molecule in sim box  = ',f10.3 ,' mmol/gram')") wzeo/6.023e23,1000.0/wzeo

      if (myid.eq.0) then
         write(16,"(  ' number of zeolite cells           = ',3i5,/, ' --------------------------------------------------')") nx,ny ,nz
