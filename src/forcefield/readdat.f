...
! -- variables for histograms	
      ...
      integer,allocatable::temphe(:)
      integer::zz,z,itemp,zzz
      ...
      logical:: linit, lecho, lmixlb, lmixjo,lsetup,lsolute
      logical::lprint,lverbose,lxyz,lfound
      logical,parameter::lhere(:)
...
! - set up the forcefield and the masses
      ...
      allocate(decode(nntype),lhere(nntype),temphe(nntype)
     &     ,rminee(nntype*nntype),lplace(ntmax,nntype),stat=jerr)
      if (jerr.ne.0) call cleanup('readdat: allocation failed')
      lhere = .false.
      lspecial=.false.
