...
!     kea
         ...
         if( ovrlap ) then
            write(io_output,*) ' overlap in initial configuration '
            call cleanup('')
         end if
...
!               end do
         ...
         if (lucall) then
            write(io_output,*) 'not recently checked for accuracy'
            call cleanup('')
!            do j = 1,nmolty
        ...
        do i = 1, nmolty
           ...
           do j=1,nswapb(i)
              ...
              do jbox = 1,jbox_max
                 ...
                 write(io_output,66) box1(i,j),box2(i,j),ibox, bsswap(i,j,ibox),bnswap(i,j,ibox), bnswap(i,j,ibox+nbox)
                 if (bnswap(i,j,ibox) .gt. 0.5d0) then
                    ...
                    write(io_output,63) bsswap(i,j,ibox),bnswap(i,j,ibox)
                 end if
              end do
           end do
           write(io_output,68) bnswap_in(i,1), bnswap_in(i,2)
           write(io_output,69) bnswap_out(i,1), bnswap_out(i,2)
        end do
...
! -- 06/08/09 KM
                  write(io_output,1402) nbl,baver(3,ibox,nbl), baver(1,ibox,nbl),baver(2,ibox,nbl), baver(3+nener+4*nmolty,ibox,nbl), (baver(2+nener+3*nmolty+zz,ibox,nbl),zz=1,nmolty)
               end do
               if (lmipsw) then
