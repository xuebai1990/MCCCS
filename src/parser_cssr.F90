...
CONTAINS
  ...
  SUBROUTINE readCSSR(fileCSSR,zeo,lunitcell,lhere,ztype,zcell,zunit)
    ...
    if (jerr.ne.0) then
       call cleanup('cannot open zeolite CSSR file')
    end if
    ...
    if (jerr.ne.0) call cleanup('readCSSR: allocation failed')
