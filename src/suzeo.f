...
!$$$      include 'mpi.inc'
      integer(KIND=normal_int)::imol,iunit,igtype,idi,jerr,i,j,k, ngrxt ,ngryt,ngrzt,ibox=1
...
!     if (ngrid.gt.maxtab) call cleanup('ngrid.gt.maxtab')
         ...
         dgry=zunity/ngry
         dgrz=zunitz/ngrz
         if (myid.eq.0) write(iou,"(' Tabulated zeolite potential: ',/, ' Size unit-cell zeolite: ',f7.4,' x ',f7.4,' x ',f7.4,/ '         x-dir           : ',i12,'  size: ',f7.4,/, '         y-dir           : ',i12,'  size: ',f7.4,/, '         z-dir           : ',i12,'  size: ',f7.4,/)") zunitx,zunity,zunitz, ngrx,dgrx,ngry,dgry,ngrz ,dgrz
         ...
         do igtype=0,gntype
            ...
            open(91,file=filename,form='binary',iostat=jerr, status='old')
            if (jerr.eq.0) then
...
! make a tabulated potential of the zeolite
               ...
               if (myid.eq.0) write(91) zunitx,zunity,zunitz,ngrx, ngry ,ngrz
               do i=0,ngrx-1
                  do j=0,ngry-1
                     do k=0,ngrz-1
                        egrid(i,j,k,igtype)=exzeof(dble(i)/ngrx ,dble(j)/ngry,dble(k)/ngrz,idi)
